...
/**
 * Le mode de défaillance qui tue les applications de langue : la charge de
...
describe('charge sur soixante jours', () => {
  ...
  function simuler(tauxReussite: number): number[] {
    const entrees = corpus(213)
    let progression = progressionVide()
  ...
  it('ne dépasse jamais la taille cible, même avec un enfant appliqué', () => {
    expect(Math.max(...simuler(0.9))).toBeLessThanOrEqual(options.taille)
  })

  it('ne dépasse jamais la taille cible, même avec un enfant en difficulté', () => {
    expect(Math.max(...simuler(0.4))).toBeLessThanOrEqual(options.taille)
  })

  it('continue de proposer du travail au bout de deux mois', () => {
    const tailles = simuler(0.8)
    expect(tailles.slice(-7).every((t) => t > 0)).toBe(true)
  })
  ...
  it('introduit un volume de vocabulaire crédible en deux mois', () => {
    ...
    const vus = Object.keys(progression.etats).length
    expect(vus).toBeGreaterThan(100)
    expect(vus).toBeLessThanOrEqual(213)
  })
})
```
 ...
 * plus cher que retarder d'un jour une découverte.
 */
export function composerSession(
  ...
  const placesNouveautes = Math.max(0, options.plafondNouveaux - dejaIntroduits)
